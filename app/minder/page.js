...
// ============================================================================
// MOBILE-OPTIMIZED ENGINE (v15.0 - PROGRESSIVE LOADING)
// ============================================================================
export default function MinderHub() {
  ...
  const [imagesPreloaded, setImagesPreloaded] = useState(new Set());

  // --------------------------------------------------------------------------
  // 1. OPTIMIZED DATA INGESTION (Priority-based)
  // --------------------------------------------------------------------------
  const loadBatch = useCallback(async (activeSession) => {
    try {
      // CRITICAL PATH: Load only profiles first
      ...
      if (unswipedDeck[0]?.image_url) {
        const img = new Image();
        img.onload = () => setImagesPreloaded(prev => new Set(prev).add(unswipedDeck[0].id));
        img.src = unswipedDeck[0].image_url;
      }
      ...
      // DEFERRED: Preload next 2 images after initial render
      requestIdleCallback(() => {
        unswipedDeck.slice(1, 3).forEach((t, idx) => {
          if (t.image_url) {
            setTimeout(() => {
              const img = new Image();
              img.onload = () => setImagesPreloaded(prev => new Set(prev).add(t.id));
              img.src = t.image_url;
            }, idx * 200); // Stagger preloads
          }
        });
      });
  ...
  // --------------------------------------------------------------------------
  // 2. DEFERRED SECONDARY DATA LOAD
  // --------------------------------------------------------------------------
  const loadSecondaryData = useCallback(async () => {
    if (secondaryDataLoaded) return;
  ...
  }, [supabase, profiles.length, secondaryDataLoaded]);
  ...
  useEffect(() => {
    const boot = async () => {
      ...
      // Defer secondary data until after render
      requestIdleCallback(() => {
        loadSecondaryData();
      });

      // Defer real-time subscription until after initial paint
      requestIdleCallback(() => {
    ...
    boot();
  }, [supabase, loadBatch, loadSecondaryData]);

  // --------------------------------------------------------------------------
  // 4. PROGRESSIVE IMAGE PRELOADING
  // --------------------------------------------------------------------------
  useEffect(() => {
    // When currentIndex changes, preload next 2 images
    const nextIndices = [currentIndex + 1, currentIndex + 2];
    
    nextIndices.forEach((idx, offset) => {
      if (profiles[idx] && !imagesPreloaded.has(profiles[idx].id)) {
        requestIdleCallback(() => {
          setTimeout(() => {
            const img = new Image();
            img.onload = () => setImagesPreloaded(prev => new Set(prev).add(profiles[idx].id));
            img.src = profiles[idx].image_url;
          }, offset * 300);
        });
      }
    });
  }, [currentIndex, profiles, imagesPreloaded]);
      ...
      <div className="md:hidden fixed top-0 left-0 w-full z-[500] bg-gradient-to-b from-black via-black/80 to-transparent pt-5 pb-10 px-5 pointer-events-none">
        <div className="flex justify-between items-center w-full pointer-events-auto mb-4">
          ...
          <div className="flex gap-2">
            <button onClick={() => { setMobileLeaderboardOpen(true); loadSecondaryData(); }} className="bg-black/60 border border-white/10 p-3 rounded-full backdrop-blur-md text-yellow-500 shadow-lg active:scale-90 transition-transform"><Trophy className="w-4 h-4" /></button>
            <button onClick={() => { setMobileHudOpen(true); loadSecondaryData(); }} className="bg-black/60 border border-white/10 p-3 rounded-full backdrop-blur-md text-emerald-400 shadow-lg active:scale-90 transition-transform"><Activity className="w-4 h-4" /></button>
        ...
        {mobileHudOpen && (
          <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ type: "spring", damping: 25, stiffness: 200 }} className="fixed inset-0 z-[1000] bg-black/95 backdrop-blur-3xl md:hidden p-8 flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto space-y-4 custom-scrollbar pr-2 pb-6">
               {feed.length === 0 ? (
                 <p className="text-gray-500 text-sm text-center mt-10">Loading feed...</p>
               ) : (
            ...
            <div className="flex-1 space-y-5">
               {leaderboard.length === 0 ? <p className="text-gray-500 text-sm font-bold text-center mt-20 uppercase tracking-widest">Compiling intel...</p> : leaderboard.map((leader, i) => (
                 <div key={leader.id} className="flex items-center gap-5 bg-gradient-to-r from-white/10 to-transparent p-5 rounded-[2rem] border border-white/10 shadow-lg">
                   ...
                   <img src={leader.image_url} alt="leader" className="w-16 h-16 rounded-full object-cover border-2 border-yellow-500/50 shadow-lg" loading="lazy" />
                   <div>
              ...
              <AnimatePresence>
                {visibleCards.map((card) => (
                  <SwipeCard 
                    ...
                    imagePreloaded={imagesPreloaded.has(card.target.id)}
                  />
                ))}
              </AnimatePresence>
            </div>
...
// ============================================================================
// OPTIMIZED SWIPE CARD (Mobile-First)
// ============================================================================
const SwipeCard = React.memo(({ target, isTop, depthIndex, session, isOwnCard, existingSwipe, onExecuteSwipe, imagePreloaded }) => {
  const x = useMotionValue(0);
  const controls = useAnimation();
  
  // Defer red flag calculation for background cards
  const { score: redFlagScore, loading: scoreLoading } = useOptimizedRedFlag(target, isTop);
    ...
    >
      ...
      {imagePreloaded ? (
        ...
        <div className="absolute inset-0 w-full h-full bg-gray-900 animate-pulse flex items-center justify-center">
          <Radar className="w-12 h-12 text-pink-500/30 animate-spin" />
        </div>
      )}
...
// ============================================================================
// OPTIMIZED RED FLAG HOOK (Deferred for background cards)
// ============================================================================
function useOptimizedRedFlag(target, isTopCard) {
  ...
  useEffect(() => {
    ...
    const calculateScore = async () => {
      ...
      // Only calculate for top card immediately
      // Background cards wait 1 second
      const delay = isTopCard ? 0 : 1000;
