...
// ============================================================================
// THE FLAGSHIP ENGINE (v11.0.0 - AI HYBRID ROUTING + INFINITE LOOP)
// ============================================================================
export default function MinderHub() {
  ...
  useEffect(() => {
    const bootSystem = async () => {
      ...
      // Sort Deck: Put swiped cards at the bottom of the stack
      const sortedDeck = [...rawTargets].sort((a, b) => {
        const aSwiped = swipeMap.has(a.id) ? 1 : 0;
        const bSwiped = swipeMap.has(b.id) ? 1 : 0;
        return bSwiped - aSwiped; 
      });

      // Inject unique render keys for infinite looping
      setTargets(sortedDeck.map(t => ({ ...t, renderKey: `${t.id}-${Date.now()}` })));
      setLoading(false);

      // Realtime Websocket Uplink
        ...
        {authModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[2000] flex items-center justify-center p-6 bg-black/80 backdrop-blur-md">
            <motion.div initial={{ scale: 0.95, y: 20 }} animate={{ scale: 1, y: 0 }} className="w-full max-w-sm bg-[#0a0a0f] border-2 border-red-600/50 rounded-[2rem] p-8 shadow-[0_0_100px_rgba(220,38,38,0.4)] flex flex-col items-center text-center relative overflow-hidden">
              ...
              <button onClick={() => setAuthModal(false)} className="mt-6 text-[10px] text-gray-500 hover:text-white uppercase tracking-[0.3em] font-black z-10 p-2">DISMISS WARNING</button>
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
        ...
        <div className="flex-1 w-full flex items-center justify-center relative my-2 md:my-0 z-10">
          {loading ? (
            ...
            <div className="absolute inset-2 md:relative md:w-full md:max-w-[440px] md:h-[750px] flex items-center justify-center">
              <AnimatePresence>
                {targets.slice(-3).map((target, index, arr) => {
        ...
        <div className="flex justify-between items-end gap-3">
          ...
          <div className="text-center bg-black/80 p-3 rounded-2xl border border-white/10 backdrop-blur-md shadow-2xl flex flex-col items-center min-w-[70px]">
            ...
            <div className="text-3xl font-black text-white leading-none">
              {scoreLoading ? (
                <Activity className="w-6 h-6 animate-spin text-pink-600 my-1" />
              ) : (
                `${redFlagScore}%`
              )}
            </div>
...
function useHybridRedFlag(target) {
  ...
  useEffect(() => {
    ...
    const calculateScore = async () => {
      ...
      // PHASE 1: Neural Uplink (GLM-4 API)
      try {
      ...
      // PHASE 2: Offline V-Smart Fallback
      let score = 15;
