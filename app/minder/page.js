...
// ============================================================================
// THE FLAGSHIP ENGINE (v14.2 - OPTIMIZED FOR MOBILE)
// Changes applied:
// - loadBatch default size reduced and progressive fetchMore implemented
// - boot() split into critical + deferred phases via requestIdleCallback
// - removed expensive SVG turbulence noise (use static noise PNG)
// - images are loaded lazily + decoded async and requested with a size/quality hint
// - useHybridRedFlag only runs for the top card (reduces API calls)
// - effect dependency fixed (removed profiles.length) to avoid reboots
// - realtime channel subscription created in deferred phase and properly cleaned up
// - minimal re-renders via staged state updates
// ============================================================================
...
export default function MinderHub() {
  ...
  const visibleBatchSize = 12; // initial batch size (mobile-friendly)
  const prefetchAhead = 4; // fetch next batch when within this many cards

  // --------------------------------------------------------------------------
  // IMAGE HELPER - produce an optimized URL if backend supports query params
  // --------------------------------------------------------------------------
  const getOptimizedImageUrl = (url) => {
    if (!url) return "";
    try {
      // common supabase storage pattern: contains '/storage/v1'
      if (url.includes("/storage/v1/")) {
        // supabase render pattern
        // note: some deployments accept this pattern, if not, the browser params will be ignored
        return `${url}?width=800&quality=70&format=webp`;
      }
      // if img CDN supports params on query, you can append fallback resize params
      // otherwise return original but we still use lazy + decoding hints
      return url;
    } catch {
      return url;
    }
  ...
  // --------------------------------------------------------------------------
  // 1. DATA INGESTION (loadBatch + fetchMore)
  // --------------------------------------------------------------------------
  const loadBatch = useCallback(async (activeSession, limit = visibleBatchSize) => {
    try {
      ...
      // only pre-decode two images to avoid decoding stall
      unswipedDeck.slice(0, 2).forEach(t => {
        if (t.image_url) {
          const img = new Image();
          img.decoding = "async";
          img.src = getOptimizedImageUrl(t.image_url);
        }
      });

      // return deck so caller can set state in a single update
      return { unswipedDeck, swipeMap };
    } catch (err) {
      console.error("DATA LAYER FAILED:", err);
      return { unswipedDeck: [], swipeMap: new Map() };
    }
  }, [supabase]);

  // fetch more when user is close to end of deck
  const fetchMore = useCallback(async (activeSession) => {
    if (loadingMore) return;
    setLoadingMore(true);
    try {
      // fetch next set (keep same page size)
      const { data: rawProfiles, error } = await supabase
        .from('minder_targets')
        .select('*')
        .order('created_at', { ascending: false })
        .range(profiles.length, profiles.length + visibleBatchSize - 1);

      if (error) throw error;
      if (rawProfiles && rawProfiles.length) {
        // preload first image only
        rawProfiles.slice(0, 1).forEach(t => {
          if (t.image_url) {
            const img = new Image();
            img.decoding = "async";
            img.src = getOptimizedImageUrl(t.image_url);
  ...
  }, [supabase, profiles.length, loadingMore]);

  // --------------------------------------------------------------------------
  // 2. SYSTEM INITIALIZATION - staged (critical + deferred)
  // --------------------------------------------------------------------------
  useEffect(() => {
    let channelRef = null;
    ...
    const boot = async () => {
      try {
        // Phase 1 - critical work: session + initial small batch
        const { data: { session: activeSession } } = await supabase.auth.getSession();
        if (isCancelled) return;
        setSession(activeSession);

        const { unswipedDeck, swipeMap } = await loadBatch(activeSession, visibleBatchSize);
        if (isCancelled) return;

        // single-state updates for initial UX
        setUserSwipes(swipeMap);
        setProfiles(unswipedDeck);
        setCurrentIndex(0);
        setLoading(false);

        // Phase 2 - deferred (non-critical): feed, leaderboard, realtime
        rIC(async () => {
          if (isCancelled) return;
          ...
          try {
            ...
            if (feedData && !isCancelled) {
              setFeed(feedData.map(t => ({ id: `init-${t.id}`, text: `> DOSSIER ACTIVE: [${t.alias}]`, color: 'text-gray-500' })));
            }
          } catch (e) {
            console.warn("deferred feed load failed", e);
          }

          try {
            // leaderboard aggregation: fetch smash counts then top targets
            const { data: smashCounts } = await supabase
              .from('minder_swipes')
              .select('target_id')
              .eq('action', 'SMASH');

            if (smashCounts && !isCancelled) {
              const counts = smashCounts.reduce((acc, s) => { acc[s.target_id] = (acc[s.target_id] || 0) + 1; return acc; }, {});
              ...
              if (topIds.length) {
                ...
                if (targets && !isCancelled) {
                  setLeaderboard(targets.map(t => ({ ...t, score: counts[t.id] })).sort((a, b) => b.score - a.score));
                }
              }
            }
          } catch (e) {
            console.warn("deferred leaderboard failed", e);
          }

          try {
            // subscribe to realtime events (deferred)
            channelRef = supabase.channel(`minder_live_${Date.now()}`)
              .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'minder_swipes' }, async (payload) => {
                if (payload.new.swiper_id === (session?.user?.id)) return;
  ...
  }, [supabase, loadBatch]); // <-- profiles.length removed intentionally
  ...
  const executeSwipe = useCallback((direction, targetId, isOwnCard) => {
    ...
    // update userSwipes immutably
    setUserSwipes(prev => {
      const copy = new Map(prev);
      copy.set(targetId, action);
      return copy;
    });
  ...
  // auto fetchMore when user near end of deck
  useEffect(() => {
    if (profiles.length === 0) return;
    if (profiles.length - currentIndex <= prefetchAhead) {
      fetchMore(session);
    }
  }, [currentIndex, profiles.length, fetchMore, session]);

  const visibleCards = useMemo(() => profiles.slice(currentIndex, currentIndex + 3).map((target, idx) => ({
    target,
    relativeIndex: idx,
    isTop: idx === 0
  })).reverse(), [profiles, currentIndex]);
  ...
  return (
    <div className="h-[100dvh] bg-[#000000] text-white overflow-hidden flex flex-col md:flex-row font-mono relative touch-none select-none">
      {/* PREMIUM BACKGROUND (static noise PNG instead of SVG turbulence) */}
      <div className="fixed inset-0 pointer-events-none z-0 overflow-hidden">
        ...
        {/* replaced expensive SVG filter with static noise image */}
        <div className="absolute inset-0 opacity-[0.03] mix-blend-overlay" style={{ backgroundImage: 'url("/static/noise-plain.png")' }} />
            ...
            <div className="flex-1 space-y-5">
              {leaderboard.length === 0 ? <p className="text-gray-500 text-sm font-bold text-center mt-20 uppercase tracking-widest">Compiling intel...</p> : leaderboard.map((leader, i) => (
                <div key={leader.id} className="flex items-center gap-5 bg-gradient-to-r from-white/10 to-transparent p-5 rounded-[2rem] border border-white/10 shadow-lg">
                  ...
                  <img src={getOptimizedImageUrl(leader.image_url)} alt="leader" className="w-16 h-16 rounded-full object-cover border-2 border-yellow-500/50 shadow-lg" loading="lazy" decoding="async" />
                  <div>
        ...
        <div className="relative w-full h-[100dvh] md:h-[750px] md:max-w-[420px] flex items-center justify-center md:my-0 mt-0 pb-10 md:pb-0">
          {loading ? (
            ...
            <div className="text-center w-[90%] md:w-full bg-[#050505] p-12 border border-white/10 rounded-3xl backdrop-blur-2xl shadow-2xl relative overflow-hidden">
              <div className="absolute inset-0 bg-[url('/static/noise-plain.png')] opacity-20 mix-blend-overlay" />
...
// - small optimizations applied: GPU promote card, image lazy-loading,
//   redflag only for top card, fewer pre-decode images
// ============================================================================
const SwipeCard = React.memo(({ target, isTop, depthIndex, session, isOwnCard, existingSwipe, onExecuteSwipe }) => {
  const x = useMotionValue(0);
  const controls = useAnimation();

  // only run heavy redflag hook for the top card
  const { score: redFlagScore, loading: scoreLoading } = useHybridRedFlag(isTop ? target : null);
  ...
  return (
    ...
    >
      ...
      <img
        src={getOptimizedImageUrl(target?.image_url)}
        alt={safeAlias}
        loading="lazy"
        decoding="async"
        draggable={false}
...
// ============================================================================
// HYBRID AI HOOK (optimized to early return if target is null)
// ============================================================================
function useHybridRedFlag(target) {
  ...
  useEffect(() => {
    let isMounted = true;
    if (!target) {
      setData({ score: 0, loading: false });
      return () => { isMounted = false; };
    }

    const calculateScore = async () => {
      if (!isMounted) return;
      setData({ score: 0, loading: true });
      ...
      // Local heuristic fallback (lightweight)
      let score = 10;
      const bioLower = String(target.bio || "").toLowerCase();
      const wordCount = bioLower.split(/\s+/).filter(Boolean).length;
      ...
      if (wordCount > 10 && (pronounCount / (wordCount || 1)) > 0.15) score += 10;
