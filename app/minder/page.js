...
// ------------------------------------------------------------------               
// THE GRID CONTROLLER (v6.0.0 - BULLETPROOF & OPTIMISTIC)
// ------------------------------------------------------------------
export default function MinderHub() {
  ...
  const fetchTargets = useCallback(async (currentOffset = 0, currentSession = session) => {
    ...
    try {
      ...
      if (data && data.length > 0) {
        // Force images to cache instantly
  ...
  useEffect(() => {
    const initializeSystem = async () => {
      ...
      // PRE-LOAD RECENT HISTORY TO AVOID BLANK FEED
      ...
      const channel = supabase.channel(channelId)
        .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'minder_swipes' }, async (payload) => {
           // We rely on Optimistic UI below, but this catches OTHER people's swipes
  ...
  // 3. OPTIMISTIC SWIPE ENGINE (FIXES BLANK LOGS)
  const processSwipe = async (direction, targetId, isOwnCard = false) => {
    ...
    // Wait for the fly-away animation to complete before popping array
    setTimeout(() => {
      setTargets(prev => {
        const newDeck = [...prev];
        newDeck.pop(); 
    ...
    // INSTANT UI INJECTION (Does not wait for Supabase Realtime)
    setFeed(prev => [{ 
      id: `opt-${Date.now()}`, 
  ...
  return (
    <div className="h-[100dvh] bg-[#010103] text-white overflow-hidden flex flex-col md:flex-row font-mono relative">
      ...
      <AnimatePresence>
        {mobileHudOpen && (
          <motion.div 
            ...
            className="fixed inset-0 z-[300] bg-black/98 backdrop-blur-3xl md:hidden p-8 flex flex-col"
          >
      ...
      <div className="hidden md:flex flex-col w-[480px] bg-black/90 backdrop-blur-3xl border-r border-pink-600/20 p-12 z-10 shadow-[60px_0_150px_rgba(0,0,0,1)] relative overflow-hidden h-full">
        ...
        <div className="flex items-center gap-6 text-pink-500 mb-12 pb-10 border-b border-pink-900/40 relative z-10 pt-10">
          ...
          <div>
            ...
            <p className="text-xs text-pink-800 font-black mt-3 tracking-[0.4em]">ENCRYPTED UPLINK</p>
          </div>
        </div>
        ...
        <div className="pt-12 border-t border-pink-600/20 mt-10 relative z-10">
           <Link href="/minder/enroll">
             <button className="w-full py-7 bg-pink-600 text-white text-base font-black tracking-[0.3em] hover:bg-pink-500 transition-all shadow-[0_0_80px_rgba(219,39,119,0.5)] group flex items-center justify-center gap-5 rounded-[2rem] border-b-4 border-pink-900 active:border-b-0 active:translate-y-1">
               <Zap className="w-7 h-7 group-hover:rotate-12 transition-transform fill-current text-white" /> INJECT DOSSIER
             </button>
           </Link>
        </div>
      </div>

      {/* --- FLUID MAIN GRID CENTER --- */}
      <div className="flex-1 flex flex-col relative z-10 p-6 md:p-10 h-full overflow-hidden">
        ...
        <div className="flex justify-between items-start w-full shrink-0 relative z-[200]">
           ...
           <div className="flex flex-col items-end pointer-events-none">
             <h1 className="text-5xl md:text-6xl font-black italic tracking-tighter text-white drop-shadow-[0_0_40px_rgba(219,39,119,0.6)]">
               MINDER<span className="text-pink-600">_</span>
             </h1>
             <p className="text-[9px] md:text-[10px] font-black text-pink-500 uppercase tracking-[0.3em] bg-pink-900/30 px-4 py-1.5 rounded-full border border-pink-500/20 mt-3 backdrop-blur-md shadow-inner">
               LAWSUIT PROTOCOL ACTIVE
             </p>
           </div>
        </div>

        {/* FLUID TARGET VIEWPORT - FORCES HEIGHT ON MOBILE */}
        <div className="flex-1 w-full flex items-center justify-center relative my-4 z-10 min-h-[500px]">
          {loading ? (
        ...
        <div className="shrink-0 flex flex-col items-center gap-3 opacity-30 select-none pb-2 pointer-events-none z-0">
...
const SwipeCard = React.memo(({ target, isTop, depthIndex, session, isOwnCard, onSwipe, onForceMatch }) => {
  ...
  return (
    ...
    >
      ...
      {isOwnCard && isTop && (
        <div className="absolute top-8 left-0 w-full bg-yellow-500 text-black py-2.5 font-black text-center tracking-[0.4em] text-[10px] uppercase z-30 shadow-2xl flex items-center justify-center gap-4">
      ...
      <div className="mt-auto w-full p-6 md:p-8 flex flex-col gap-4 bg-gradient-to-t from-[#010103] via-[#010103]/98 to-transparent z-10 shrink-0 pointer-events-auto">
        <div className="flex justify-between items-end">
          <div className="flex-1 min-w-0 pr-4">
            ...
            {session ? (
              <div className="text-xs md:text-sm font-black text-white bg-pink-600 w-fit px-4 py-2.5 mt-3 rounded-xl shadow-[0_0_30px_rgba(219,39,119,0.8)] flex items-center gap-3 border border-pink-400/50">
          ...
          <div className="mt-2 flex gap-3" onPointerDown={(e) => e.stopPropagation()}>
             {isOwnCard ? (
               ...
               <>
                 ...
                 <button onClick={() => triggerSwipeAnimation('right')} className="flex-1 relative group overflow-hidden rounded-[1.2rem] bg-green-950/60 border border-green-500/30 text-green-500 font-black text-xs py-4 md:py-5 transition-all hover:bg-green-900/80 hover:border-green-400 hover:shadow-[0_0_40px_rgba(34,197,94,0.5)] active:scale-95 active:translate-y-1 z-50">
